...
// Treasury-specific concerns - shows only concerns reported to Treasury Office
...
export default function TreasuryConcerns() {
  ...
  const loadConcerns = async (silent = false) => {
    try {
      ...
      if (res.success) {
        // Filter to only show concerns for Treasury Office
        const treasuryConcerns = (res.concerns || []).filter(concern =>
          concern.reportTo === 'Treasury Office'
        );
        setConcerns(treasuryConcerns);
      }
    } catch (error) {
      console.error('Error loading treasury concerns:', error);
    } finally {
  ...
  if (loading) {
    return <div className="text-center py-[60px] text-[#FFD41C]">Loading treasury concerns...</div>;
  }
    ...
    <div className="h-full flex flex-col">
        ...
        <div style={{ marginBottom: '20px' }}>
          ...
          <p className="text-[13px] text-[rgba(251,251,251,0.6)] m-0">
            {filteredConcerns.length > 0
              ...
              : `Concerns and feedback related to Treasury Office • Total: ${concerns.length}`
            }
          </p>
        </div>
      ...
      {concerns.length === 0 ? (
        <div className="text-center py-[60px] text-[rgba(251,251,251,0.5)]">
          ...
          <div>No treasury concerns found</div>
        </div>
      ) : filteredConcerns.length === 0 ? (
        <div className="text-center py-[60px] text-[rgba(251,251,251,0.5)]">
          <div className="text-5xl mb-4">🔍</div>
