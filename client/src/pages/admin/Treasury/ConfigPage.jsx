// Treasury Admin Configurations
// Treasury-specific export options: Transactions, Merchants, Treasury Logs, Treasury Concerns

import React, { useState, useEffect } from 'react';
import api from '../../../utils/api';

export default function TreasuryConfigurationsPage() {
  ...
  // Treasury-specific export types
  ...
  const handleManualExport = async () => {
    ...
    try {
      ...
      const result = await api.post('/admin/configurations/manual-export-treasury', {
        exportTypes: configurations.autoExport.exportTypes
      });

      showAlert('success', `Treasury data exported successfully! (${result.totalRecords} total records)`);

      if (result.downloadUrl) {
        const link = document.createElement('a');
        link.href = result.downloadUrl;
        link.download = result.fileName || `treasury_export_${new Date().toISOString().split('T')[0]}.zip`;
    ...
    <div className="h-full flex flex-col">
      ...
      <div style={{ marginBottom: '24px', borderBottom: '2px solid rgba(255,212,28,0.2)', paddingBottom: '16px' }}>
        <h2 className="text-2xl font-bold text-[#FFD41C] m-0 mb-2 flex items-center gap-[10px]">
          <span>⚙️</span> Treasury Configurations
        </h2>
        <p className="text-[13px] text-[rgba(251,251,251,0.6)] m-0">
          Configure automated treasury data exports and system settings
        </p>
      </div>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px' }}>
            <div>
              <h3 style={{ color: '#FFD41C', fontSize: '20px', fontWeight: 700, margin: '0 0 8px 0', display: 'flex', alignItems: 'center', gap: '10px' }}>
                <span>📊</span> Automated Treasury Export
              </h3>
              <p style={{ color: 'rgba(251,251,251,0.6)', fontSize: '14px', margin: 0, lineHeight: '1.5' }}>
                System automatically exports treasury data as ZIP files for backup and compliance.
          ...
          <div style={{
            ...
            <p style={{ color: 'rgba(251,251,251,0.6)', fontSize: '13px', marginBottom: '16px' }}>
              Download a complete ZIP file with all treasury data: transactions, merchants, logs, and concerns.
            </p>
            <button
            ...
            >
              {loading ? '⏳ Exporting...' : '📥 Download Treasury Data (ZIP)'}
            </button>
          </div>
        </div>
      ...
      {showExportModal && (
        <ConfigModal
          ...
          exportTypes={TREASURY_EXPORT_TYPES}
          loading={loading}
