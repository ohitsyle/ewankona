// Motorpool Admin Configurations
// Motorpool-specific export options: Drivers, Routes, Trips, Shuttles, Driver Phones, Motorpool Logs, Motorpool Concerns

import React, { useState, useEffect } from 'react';
import api from '../../../utils/api';

export default function MotorpoolConfigurationsPage() {
  ...
  // Motorpool-specific export types
  const MOTORPOOL_EXPORT_TYPES = [
    { value: 'Drivers', icon: '👨‍✈️', label: 'Drivers' },
    { value: 'Routes', icon: '🗺️', label: 'Routes' },
    { value: 'Trips', icon: '📍', label: 'Trips' },
    { value: 'Shuttles', icon: '🚐', label: 'Shuttles' },
    { value: 'Phones', icon: '📱', label: 'Driver Phones' },
    { value: 'Logs', icon: '📋', label: 'Motorpool Logs' },
    { value: 'Concerns', icon: '💬', label: 'Motorpool Concerns' }
  ];
  ...
  const loadExportHistory = async () => {
    try {
      const data = await api.get('/admin/configurations/export-history?adminRole=motorpool');
  ...
  const loadScheduledExports = async () => {
    try {
      const data = await api.get('/admin/configurations/scheduled-exports?adminRole=motorpool');
  ...
  const handleManualExport = async () => {
    ...
    try {
      ...
      if (!configurations.autoExport.exportTypes || configurations.autoExport.exportTypes.length === 0) {
        showAlert('error', 'Please select at least one export type in the configuration settings');
        setLoading(false);
        return;
      }

      const result = await api.post('/admin/configurations/manual-export-motorpool', {
        exportTypes: configurations.autoExport.exportTypes
      });

      showAlert('success', `Motorpool data exported successfully! (${result.totalRecords} total records)`);

      if (result.downloadUrl) {
        const link = document.createElement('a');
        link.href = result.downloadUrl;
        link.download = result.fileName || `motorpool_export_${new Date().toISOString().split('T')[0]}.zip`;
    ...
    <div className="h-full flex flex-col">
      ...
      <div style={{ marginBottom: '24px', borderBottom: '2px solid rgba(255,212,28,0.2)', paddingBottom: '16px' }}>
        <h2 className="text-2xl font-bold text-[#FFD41C] m-0 mb-2 flex items-center gap-[10px]">
          <span>⚙️</span> Motorpool Configurations
        </h2>
        <p className="text-[13px] text-[rgba(251,251,251,0.6)] m-0">
          Configure automated motorpool data exports and system settings
        </p>
      </div>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px' }}>
            <div>
              <h3 style={{ color: '#FFD41C', fontSize: '20px', fontWeight: 700, margin: '0 0 8px 0', display: 'flex', alignItems: 'center', gap: '10px' }}>
                <span>📊</span> Automated Motorpool Export
              </h3>
              <p style={{ color: 'rgba(251,251,251,0.6)', fontSize: '14px', margin: 0, lineHeight: '1.5' }}>
                System automatically exports motorpool data as ZIP files for backup and compliance.
          ...
          <div style={{
            ...
            <h4 style={{ color: '#FBFBFB', fontSize: '16px', fontWeight: 700, marginBottom: '8px' }}>
              Export Motorpool Data Now
            </h4>
            <p style={{ color: 'rgba(251,251,251,0.6)', fontSize: '13px', marginBottom: '16px' }}>
              Download a complete ZIP file with all motorpool data: drivers, routes, trips, shuttles, phones, logs, and concerns.
            </p>
            <button
            ...
            >
              {loading ? '⏳ Exporting...' : '📥 Download Motorpool Data (ZIP)'}
            </button>
          </div>
        </div>

        {/* Scheduled Exports Archive */}
        <div style={{
          background: 'rgba(255,255,255,0.05)',
          borderRadius: '16px',
          border: '1px solid rgba(255,212,28,0.2)',
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
            <div>
              <h3 style={{ color: '#FFD41C', fontSize: '18px', fontWeight: 700, margin: '0 0 4px 0', display: 'flex', alignItems: 'center', gap: '8px' }}>
                <span>🗄️</span> Scheduled Export Archive
              </h3>
              <p style={{ color: 'rgba(251,251,251,0.5)', fontSize: '12px', margin: 0 }}>
                Auto-exported files saved from scheduled exports
              </p>
            </div>
            <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                ...
                />
                <span style={{ color: 'rgba(251,251,251,0.5)' }}>→</span>
                <input
                  type="date"
          ...
          {scheduledExports.length === 0 ? (
            <div style={{
              ...
              <p style={{ margin: 0 }}>No scheduled exports yet</p>
              <p style={{ margin: '8px 0 0 0', fontSize: '12px' }}>
                Configure auto-export schedule to start archiving exports
              </p>
            </div>
          ) : (
            <div style={{
                    ...
                    >
                      <div style={{ flex: 1 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '6px' }}>
                          ...
                          <span style={{ color: '#FFD41C', fontWeight: 700, fontSize: '15px' }}>
                            {exp.exportType || 'Scheduled Export'}
                          </span>
                          <span style={{
                            padding: '4px 10px',
                            borderRadius: '6px',
                        ...
                        <div style={{ display: 'flex', gap: '20px', fontSize: '13px', color: 'rgba(251,251,251,0.5)' }}>
        ...
        {exportHistory.length > 0 && (
          <div style={{
            ...
            <div style={{ marginBottom: '16px' }}>
              <h3 style={{ color: '#FFD41C', fontSize: '18px', fontWeight: 700, margin: '0 0 4px 0', display: 'flex', alignItems: 'center', gap: '8px' }}>
                <span>📜</span> Manual Export History
              </h3>
              <p style={{ color: 'rgba(251,251,251,0.5)', fontSize: '12px', margin: 0 }}>
                Recent manual exports triggered by administrators
              </p>
            </div>
            <div style={{
              maxHeight: '400px',
      ...
      {showExportModal && (
        <ConfigModal
          ...
          exportTypes={MOTORPOOL_EXPORT_TYPES}
          loading={loading}
...
function DateRangeModal({ dateRange, setDateRange, customStartDate, setCustomStartDate, customEndDate, setCustomEndDate, loading, onExport, onCancel }) {
  return (
    <div
      onClick={onCancel}
      style={{
...
function ConfigModal({ configurations, setConfigurations, exportTypes, loading, onSave, onCancel }) {
  return (
    <div
      onClick={onCancel}
      style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '10px' }}>
              {exportTypes.map((type) => (
                <label
                  ...
                  onMouseLeave={(e) => {
                    e.currentTarget.style.background = configurations.autoExport.exportTypes?.includes(type.value)
                      ? 'rgba(255, 212, 28, 0.15)'
                      : 'rgba(255, 255, 255, 0.05)';
                  }}
                >
                  ...
                  <span style={{ color: '#FBFBFB', fontSize: '14px', fontWeight: 600 }}>{type.label}</span>
                </label>
              ))}
            </div>
          </div>
